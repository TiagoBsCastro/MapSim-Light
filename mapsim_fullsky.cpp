...
void Hcenter(double *ra, double *dec, double daperture){
  // ra should go from 0 to ddx in radiants
  // dec also     from 0 to ddy in radiants
  *dec = *dec + M_PI/2. - daperture/4.;
  *ra = *ra + 2*M_PI - daperture/2.;
  if(*ra>2*M_PI) *ra = *ra - 2*M_PI;
}   
...
int main(){ 
  ...
  cout << "   -                including Healpix  ....             - " << endl;
  cout << "   -                                                    - " << endl;
  cout << "   -               collapsing one dimension             - " << endl;
  cout << "   ------------------------------------------------------ " << endl;
  
  bool healpixON = false;
  // check if Healpix modality is on ... 
  std:: string filehealpix = "healpix.d";  
  std:: ifstream infilehealpix;
  infilehealpix.open(filehealpix.c_str());
  if(infilehealpix.is_open()){
    std:: cout << " Healpix modality is on!!! " << std:: endl;
    healpixON = true;
    infilehealpix.close();
  }    
  ...
  if(infileplstart.is_open()){
    std:: cout << " I will read the restart file file" << std:: endl;
    std:: cout << fileplstart << std:: endl;
  ...
  if(nfiles<0){
    // read a files with the numnber of files per snaphost in order of snaps
  ...
  for(int nsnap=iplrestart;nsnap<nsnaps;nsnap++){
    ...
    // heapix map resolution
    int healp_nring = 4*truenpix-1;
    int healp_npix = 12*truenpix*truenpix;
    //
    // healpixON // allocate only if necessary for each particle type
    Healpix_Map<double> healpix_map0(truenpix, RING,SET_NSIDE);   
    Healpix_Map<double> healpix_map1(truenpix, RING,SET_NSIDE);   
    Healpix_Map<double> healpix_map2(truenpix, RING,SET_NSIDE);   
    Healpix_Map<double> healpix_map3(truenpix, RING,SET_NSIDE);   
    Healpix_Map<double> healpix_map4(truenpix, RING,SET_NSIDE);   
    Healpix_Map<double> healpix_map5(truenpix, RING,SET_NSIDE);   
    // sum of all of them
    Healpix_Map<double> healpix_map(truenpix, RING,SET_NSIDE);   
    // initialize to 0 all the maps
    for(int l=0;l<healp_npix;l++){
      healpix_map0[l]=0.;
      healpix_map1[l]=0.;
      healpix_map2[l]=0.;
      healpix_map3[l]=0.;
    ...
    vector<double> velDisp,vmax,halfmassradius,rmax,dsub;
    // ...c as read from the catalogue
    vector<double> vxsubc, vysubc,vzsubc;
    vector<double> vxsub, vysub,vzsub;
    if(subfiles!="NO"){
      read_SUBFIND(Filesub,boxl,mfof,xfofc,yfofc,zfofc,
		   firstsubinfof,nsubinfof,
		   m200,r200,
		   msub,xsubc,ysubc,zsubc,
		   idPH,velDisp,vmax,
		   halfmassradius,rmax,groupN,
		   dsub, vxsubc, vysubc,  vzsubc);
      // std:: cout << " read subfind ... " << std:: endl;
      // exit(1);
    }
    int nfof = mfof.size();
    int nsub = msub.size();
    ...
    */
    ...
    vector<double> halfmassradiussub,distsub,vradsub;
	...
	if(subfiles!="NO"){	
	  ...
	  for(int l=0;l<nsub;l++){
	    ...
	    vxsubc[l] = sgnX[rcase]*vxsubc[l];
	    vysubc[l] = sgnY[rcase]*vysubc[l];
	    vzsubc[l] = sgnZ[rcase]*vzsubc[l];
	    // wrapping periodic condition 
	  ...
	  for(int l=0;l<nfof;l++){
	    ...
	    if(di>=blD[nsnap] && di<blD2[nsnap]){
	      ...
	      if(healpixON){
		if(fabs(rai*cos(deci))<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){
		  double xa = zfof[l];
		  double ya = xfof[l]-0.5;
		  double za = yfof[l]-0.5;
		  v3.Set(xa,ya,za);  		
		  int fofpix = healpix_map0.vec2pix(v3);
		  point = healpix_map0.pix2ang(fofpix);  /// coordinate of the pixel
		  double lat = M_PI/2. - point.theta;
		  double lon = point.phi;		  

		  idfof.push_back(l);
		  mfofsel.push_back(mfof[l]);
		  m200sel.push_back(m200[l]);
		  r200sel.push_back(r200[l]);
		  fofx.push_back(lon);  // need to be in radiants
		  fofy.push_back(lat); // need to be in radiants
		  fofz.push_back(di);  // need to be checked!
		  zifof.push_back(getY(dl,zl,di)); // need to be checked!
		}		
	      }else{
	  ...
	  for(int l=0;l<nsub;l++){
		...
		if(fabs(rai*cos(deci))<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){
		  double xa = zsub[l];
		  double ya = xsub[l]-0.5;
		  double za = ysub[l]-0.5;
		  v3.Set(xa,ya,za);  		
		  int subpix = healpix_map0.vec2pix(v3);
		  point = healpix_map0.pix2ang(subpix);  /// coordinate of the pixel
		  double lat = M_PI/2. - point.theta;
		  double lon = point.phi;		  
		  idsub.push_back(l);
		  ...
		  distsub.push_back(dsub[l]);
		  double velrad = (vxsub[l]*(xsub[l]-0.5) + vysub[l]*(ysub[l]-0.5) + vzsub[l]*zsub[l])/
		    sqrt(pow(xsub[l]-0.5,2) + pow(ysub[l],2) + pow(zsub[l],2));		    
		  vradsub.push_back(velrad);
                  halfmassradiussub.push_back(halfmassradius[l]);
                  msubsel.push_back(msub[l]);
                  subx.push_back(lon);  // need to be in radiants
                  suby.push_back(lat); // need to be in radiants
                  subz.push_back(di); // need to be checked!
                  zisub.push_back(getY(dl,zl,di)); // need to be checked!
		}		
	      }else{
		if(fabs(rai)<=(truefovradiants*0.5+bufferrad) && fabs(deci)<=(truefovradiants*0.5+bufferrad)){	  
		  idsub.push_back(l);
	...
	for(int l=0;l<n0;l++){
	  ...
	  if(di>=blD[nsnap] && di<blD2[nsnap]){
	    ...
	    if(healpixON){
	      if(fabs(rai*cos(deci))<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  
		xs.push_back(zz0[l]);
		ys.push_back(xx0[l]-0.5);
		rs.push_back(yy0[l]-0.5);
	      }
	    }else{
		...
		// Tiago!
		// xs.push_back(deci/fovinradians+0.5);
		// ys.push_back(rai/fovinradians+0.5)
	      }
	    }
	  }
	}
	...
	if(healpixON){
	  int id_ring;
	  for(int l=0;l<totPartxy0;l++){
	    v3.Set(xs[l],ys[l],rs[l]);  
	    id_ring= healpix_map0.vec2pix(v3);
	    healpix_map0[id_ring]+=1;  /// should assign the mass ... 
	  }
	}else{
	  if(totPartxy0>0){
	...
	for(int l=0;l<n1;l++){
	  ...
	  if(di>=blD[nsnap] && di<blD2[nsnap]){
	    ...
	    if(healpixON){
	      if(fabs(rai*cos(deci))<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  
		xs.push_back(zz1[l]);
		ys.push_back(xx1[l]-0.5);
		rs.push_back(yy1[l]-0.5);
	      }
	    }else{
	      if(fabs(rai)<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  
		double fovinunitbox = fovradiants*di/(data.boxsize/1.e+3);
	...
	if(healpixON){
	  int id_ring;
	  for(int l=0;l<totPartxy1;l++){
	    v3.Set(xs[l],ys[l],rs[l]);  
	    id_ring= healpix_map1.vec2pix(v3);
	    healpix_map1[id_ring]+=1;  /// should assign the mass ... 
	  }
	}else{	  
	  if(totPartxy1>0){
	...
	for(int l=0;l<n2;l++){
	  ...
	  if(di>=blD[nsnap] && di<blD2[nsnap]){
	    ...
	    if(healpixON){
	      if(fabs(rai*cos(deci))<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  
		xs.push_back(zz2[l]);
		ys.push_back(xx2[l]-0.5);
		rs.push_back(yy2[l]-0.5);
	      }
	    }else{
	      if(fabs(rai)<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  
		double fovinunitbox = fovradiants*di/(data.boxsize/1.e+3);
	...
	if(healpixON){
	  int id_ring;
	  for(int l=0;l<totPartxy2;l++){
	    v3.Set(xs[l],ys[l],rs[l]);  
	    id_ring= healpix_map2.vec2pix(v3);
	    healpix_map2[id_ring]+=1;  /// should assign the mass ... 
	  }
	}else{	  
	  if(totPartxy2>0){
	...
	for(int l=0;l<n3;l++){
	  ...
	  if(di>=blD[nsnap] && di<blD2[nsnap]){
	    ...
	    if(healpixON){
	      if(fabs(rai*cos(deci))<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  
		xs.push_back(zz3[l]);
		ys.push_back(xx3[l]-0.5);
		rs.push_back(yy3[l]-0.5);
	      }
	    }else{
	      if(fabs(rai)<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  
		double fovinunitbox = fovradiants*di/(data.boxsize/1.e+3);
	...
	if(healpixON){
	  int id_ring;
	  for(int l=0;l<totPartxy3;l++){
	    v3.Set(xs[l],ys[l],rs[l]);  
	    id_ring= healpix_map3.vec2pix(v3);
	    healpix_map3[id_ring]+=1;  /// should assign the mass ... 
	  }
	}else{	  
	  if(totPartxy3>0){
	...
	for(int l=0;l<n4;l++){
	  ...
	  if(di>=blD[nsnap] && di<blD2[nsnap]){
	    ...
	    if(healpixON){
	      if(fabs(rai*cos(deci))<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  	      
		xs.push_back(zz4[l]);
		ys.push_back(xx4[l]-0.5);
		rs.push_back(yy4[l]-0.5);
	      }
	    }else{
	      if(fabs(rai)<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  
		double fovinunitbox = fovradiants*di/(data.boxsize/1.e+3);
	...
	if(healpixON){
	  ...
	  for(int l=0;l<totPartxy4;l++){
	    v3.Set(xs[l],ys[l],rs[l]);  
	    id_ring= healpix_map4.vec2pix(v3);
	    healpix_map4[id_ring]+=1;  /// should assign the mass ... 
	  }
	}else{	  
	  if(totPartxy4>0){
	...
	for(int l=0;l<n5;l++){
	  ...
	  if(di>=blD[nsnap] && di<blD2[nsnap]){
	    ...
	    if(healpixON){
	      if(fabs(rai*cos(deci))<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  	      
		xs.push_back(zz5[l]);
		ys.push_back(xx5[l]-0.5);
		rs.push_back(yy5[l]-0.5);
	      }
	    }else{
	      if(fabs(rai)<=fovradiants*0.5 && fabs(deci)<=fovradiants*0.5){	  
		double fovinunitbox = fovradiants*di/(data.boxsize/1.e+3);
	...
	if(healpixON){
	  int id_ring;
	  for(int l=0;l<totPartxy5;l++){
	    v3.Set(xs[l],ys[l],rs[l]);  
	    id_ring= healpix_map5.vec2pix(v3);
	    healpix_map5[id_ring]+=1;  /// should assign the mass ... 
	  }
	}else{	  
	  if(totPartxy5>0){
      ...
      if(healpixON){
	// sum up all maps for healpix case
	for(int i=0;i<healp_npix;i++){
	  healpix_map[i]=m0*healpix_map0[i]+
	    m1*healpix_map1[i] + 
	    m2*healpix_map2[i] + 
	    m3*healpix_map3[i] + 
	    m4*healpix_map4[i] + 
    ...
    if(noSNAP==0){
      if(healpixON){
	/**
	 * write image arrays into healpix map fits format files
	 */
	if(partinplanes=="ALL"){
	  string fileoutput;
	  fileoutput = simulation+"."+snappl+".planeHealpix_"+snpix+".fits";
	  write_Healpix_map_to_fits(fileoutput, healpix_map, PLANCK_FLOAT32);
	}else{
	  string fileoutput;
	  if(ntotxy0>0){
	    fileoutput = simulation+"."+snappl+".ptype0_plane_"+snpix+".fits";
	    write_Healpix_map_to_fits(fileoutput, healpix_map0, PLANCK_FLOAT32);
	  }
	  if(ntotxy1>0){
	    fileoutput = simulation+"."+snappl+".ptype1_plane_"+snpix+".fits";
	    write_Healpix_map_to_fits(fileoutput, healpix_map1, PLANCK_FLOAT32);
	  }
	  if(ntotxy2>0){
	    fileoutput = simulation+"."+snappl+".ptype2_plane_"+snpix+".fits";
	    write_Healpix_map_to_fits(fileoutput, healpix_map2, PLANCK_FLOAT32);
	  }
	  if(ntotxy3>0){
	    fileoutput = simulation+"."+snappl+".ptype3_plane_"+snpix+".fits";
	    write_Healpix_map_to_fits(fileoutput, healpix_map3, PLANCK_FLOAT32);
	  }
	  if(ntotxy4>0){
	    fileoutput = simulation+"."+snappl+".ptype4_plane_"+snpix+".fits";
	    write_Healpix_map_to_fits(fileoutput, healpix_map4, PLANCK_FLOAT32);
	  }
	  if(ntotxy5>0){
	    fileoutput = simulation+"."+snappl+".ptype5_plane_"+snpix+".fits";
	    write_Healpix_map_to_fits(fileoutput, healpix_map5, PLANCK_FLOAT32);
	  }	  
	}
      }else{
	if(partinplanes=="ALL"){
